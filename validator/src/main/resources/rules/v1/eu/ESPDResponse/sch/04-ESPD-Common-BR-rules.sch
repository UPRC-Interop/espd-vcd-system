...
-->
		...
		<rule context="/espd:ESPDResponse/cac:ProcurementProjectLot">
			<assert test="( not(cbc:ID!='0') )">The current version of the ESPD Service does not allow for the
		...
		<!-- EXCLUSION Criteria -->
		<!--xsl:variable name="exclusion_types" select="document('../genericodes/CriteriaTypeCodes-CodeList.gc')/gc:CodeList"/--><!-- Code list of the exclusion criteria: CriteriaTypeCode-CodeList.gc -->
		<xsl:variable name="TypeCodeExclusion"
					  select="/espd:ESPDResponse/ccv:Criterion/cbc:TypeCode[starts-with(., 'CRITERION.EXCLUSION.')]"/><!-- Criterias of the XML that are EXCLUSION (those started with EXCLUSION.) -->
		<rule context="/espd:ESPDResponse">
			...
			<assert test="( not(string(cbc:ID))=false() )">The element 'espd:ESPDResponse / cbc:ID' is mandatory
			</assert>

			<assert test="( not(string(cbc:IssueDate))=false() )">The element 'espd:ESPDResponse / cbc:IssueDate' is
				mandatory
			</assert>
			...
			<!--assert test="( not(string(cbc:ContractFolderID))=false() )">The element 'espd:ESPDResponse / cbc:ContractFolderID' is mandatory </assert-->

			<!-- The ESPDResponse MUST always contain one AdditionalDocumentReference element referring to the Contract Notice (CN) published in TeD (Publications Office). -->
			<assert test="count( cac:AdditionalDocumentReference ) &gt; 0">The ESPDResponse MUST always contain one
		...
		<rule context="/espd:ESPDResponse/ccv:Criterion">
			...
			<assert test="not( (count(ccv:LegislationReference) = 0) and (contains(cbc:TypeCode, 'CRITERION.EXCLUSION.'))  )">
				All Exclusion Criteria MUST provide a value for the element LegislationReference
			</assert>
		</rule>

		<!-- Element: ccv:Criterion / RequirementGroup / cbc:ID -->
		<rule context="/espd:ESPDResponse/ccv:Criterion/ccv:RequirementGroup">
			<!-- Validates that the element exists -->
			<assert test="( not(string(cbc:ID))=false() )">The element 'cbc:ID' is mandatory</assert>
		</rule>

		<!-- Element: ccv:Criterion / RequirementGroup / Requirement / cbc:Description and cbc:ID and responseDataType -->
		<rule context="/espd:ESPDResponse/ccv:Criterion/ccv:RequirementGroup/ccv:Requirement">
			<!-- Validates that the element exists -->
			<assert test="( not(string(cbc:Description))=false() )">The element 'cbc:Description' is mandatory</assert>
			...
			<assert test="( not(string(@responseDataType))=false() )">The attribute 'responseDataType' is mandatory
			</assert>
		</rule>
		...
		<rule context="/espd:ESPDResponse/ccv:Criterion/ccv:LegislationReference">
			...
			<assert test="( not(string(ccv-cbc:JurisdictionLevelCode))=false() )">The element
		...
		<rule context="ccv:RequirementGroup/cbc:ID | ccv:RequirementGroup/ccv:Requirement/cbc:ID">
			<assert test="( not(@schemeAgencyID!='EU-COM-GROW') )">The value of the attribute 'schemeAgencyID' must be
				'EU-COM-GROW'
			</assert>
			<assert test="( not(@schemeID!='CriterionRelatedIDs') )">The value of the attribute 'schemeID' must be
				'CriterionRelatedIDs'
			</assert>
			<assert test="( not(@schemeVersionID!='1.0') )">The value of the attribute 'schemeVersionID' must be '1.0'
			</assert>
		</rule>
		...
		<rule context="/espd:ESPDResponse/cac:AdditionalDocumentReference">
			...
			<assert test="( not(string(cbc:ID))=false() )">The AdditionalDocumentReference referring to the CN published
		 ...
		 e.g. 2016/S 003-002226 -->
			<assert test="(count(cbc:ID)=0)  or not(cbc:DocumentTypeCode/text() = 'TED_CN') or ( (count(cbc:ID))=1 and (string-length(cbc:ID)=17 and substring(cbc:ID,5,3)='/S ' and substring(cbc:ID,11,1)='-'))">
