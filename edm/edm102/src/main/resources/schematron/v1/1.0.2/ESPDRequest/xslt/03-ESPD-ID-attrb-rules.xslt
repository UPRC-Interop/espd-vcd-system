...
<!--PATTERN id-attrb-rules-->
	...
	<!--RULE -->
<xsl:template match="cbc:ID | cbc:UUID | cbc:VersionID | cbc:PreviousVersionID | cbc:ContractFolderID" mode="M9" priority="1007">
...
<xsl:choose>
      ...
      <xsl:otherwise>
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="( string(.)='urn:www.cenbii.eu:transaction:biitrns070:ver3.0' )" />
      <xsl:otherwise>
        <svrl:failed-assert test="( string(.)='urn:www.cenbii.eu:transaction:biitrns070:ver3.0' )">
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
