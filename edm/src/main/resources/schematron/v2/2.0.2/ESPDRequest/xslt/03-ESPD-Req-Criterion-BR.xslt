...
<!--SCHEMA SETUP-->
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="" title="ESPD Request Criterion Business Rules">
      <xsl:comment>
...
<!--SCHEMATRON PATTERNS-->
<svrl:text>ESPD Request Criterion Business Rules</svrl:text>

<!--PATTERN BR-REQ-CR-->


	<!--RULE -->
<xsl:template match="espd:QualificationApplicationRequest" mode="M5" priority="1000">
    <svrl:fired-rule context="espd:QualificationApplicationRequest" />
    <xsl:variable name="current_Exclusion" select="cac:TenderingCriterion[starts-with(cbc:CriterionTypeCode, 'CRITERION.EXCLUSION.')]" />
    <xsl:variable name="applicationType" select="/*[1]/cbc:QualificationApplicationTypeCode" />
    <xsl:variable name="ElementUUID_Exclusion" select="if ($applicationType!='SELFCONTAINED') then document('ESPD-CriteriaTaxonomy-REGULATED.V2.0.2.xml')//cac:TenderingCriterion[starts-with(cbc:CriterionTypeCode, 'CRITERION.EXCLUSION.')]      else document('ESPD-CriteriaTaxonomy-SELFCONTAINED.V2.0.2.xml')//cac:TenderingCriterion[starts-with(cbc:CriterionTypeCode, 'CRITERION.EXCLUSION.')]" />

		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="count($ElementUUID_Exclusion) &lt;= count($current_Exclusion)" />
      <xsl:otherwise>
        <svrl:failed-assert test="count($ElementUUID_Exclusion) &lt;= count($current_Exclusion)">
    ...
    <xsl:variable name="current_Selection" select="cac:TenderingCriterion[starts-with(cbc:CriterionTypeCode, 'CRITERION.SELECTION.')]" />

		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="count($current_Selection) != 0" />
      <xsl:otherwise>
        <svrl:failed-assert test="count($current_Selection) != 0">
