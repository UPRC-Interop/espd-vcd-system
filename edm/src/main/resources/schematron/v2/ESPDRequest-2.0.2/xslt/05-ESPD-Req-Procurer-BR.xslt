...
<!--SCHEMA SETUP-->
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="" title="ESPD Request Procurer Business Rules">
      <xsl:comment>
...
<!--SCHEMATRON PATTERNS-->
<svrl:text>ESPD Request Procurer Business Rules</svrl:text>

<!--PATTERN BR-REQ-PROC-->


	<!--RULE -->
<xsl:template match="espd:QualificationApplicationRequest" mode="M6" priority="1002">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="count(cac:ContractingParty) = 1">
	...
	<!--RULE -->
<xsl:template match="cac:ContractingParty/cac:Party" mode="M6" priority="1001">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cac:ServiceProviderParty/cac:Party" mode="M6" priority="1000">
