...
<!--PATTERN BR-RESP-LEAD-->
	...
	<!--RULE -->
<xsl:template match="espd:QualificationApplicationResponse" mode="M6" priority="1003">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="(count(key('EOroleTest', 'LE'))=1 and (exists(cbc:EconomicOperatorGroupName))) or not(count(key('EOroleTest', 'LE'))=1)">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cac:TenderingCriterion[ starts-with(cbc:CriterionTypeCode, 'CRITERION.OTHER.EO_DATA.TOGETHER_WITH_OTHERS') ]" mode="M6" priority="1002">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="( count(key('EOroleTest', 'LE'))=1 and (count($togetherCriterionResponse) > 0) ) or not(count(key('EOroleTest', 'LE'))=1)">
	...
	<!--RULE -->
<xsl:template match="cac:TenderingCriterion[ starts-with(cbc:CriterionTypeCode, 'CRITERION.OTHER.EO_DATA.RELIES_ON_OTHER_CAPACITIES') ]" mode="M6" priority="1001">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="( count(key('EOroleTest', 'LE'))=1 and (count($relyCriterionResponse) > 0) ) or not(count(key('EOroleTest', 'LE'))=1)">
	...
	<!--RULE -->
<xsl:template match="cac:TenderingCriterion[ starts-with(cbc:CriterionTypeCode, 'CRITERION.OTHER.EO_DATA.SUBCONTRACTS_WITH_THIRD_PARTIES') ]" mode="M6" priority="1000">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="( count(key('EOroleTest', 'LE'))=1 and (count($subcontractorResponse) > 0) ) or not(count(key('EOroleTest', 'LE'))=1)">
