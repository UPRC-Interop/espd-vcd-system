...
<xsl:choose>
    ...
    <xsl:variable name="alpha_Selection" select="cac:TenderingCriterion[cbc:CriterionTypeCode = 'CRITERION.SELECTION.ALL']" />

		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="((count($alpha_Selection)=1) and (count($current_Selection) &lt; 2)) or (count($alpha_Selection)=0)" />
      <xsl:otherwise>
        <svrl:failed-assert test="((count($alpha_Selection)=1) and (count($current_Selection) &lt; 2)) or (count($alpha_Selection)=0)">
