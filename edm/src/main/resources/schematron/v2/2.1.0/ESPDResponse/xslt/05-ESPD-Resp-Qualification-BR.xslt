...
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="" title="ESPD Response pre-qualification system Business Rules">
      ...
      <svrl:ns-prefix-in-attribute-values prefix="espd" uri="urn:oasis:names:specification:ubl:schema:xsd:QualificationApplicationResponse-2" />
      <svrl:active-pattern>
...
<xsl:template match="espd:QualificationApplicationResponse" mode="M6" priority="1000">
    ...
    <xsl:variable name="exclusionResponses" select="$exclusionCriteria[cac:TenderingCriterionPropertyGroup/cac:TenderingCriterionProperty[cbc:ID = $allResponses and cbc:TypeCode='QUESTION']]/cbc:CriterionTypeCode" />
    <xsl:variable name="exclusionReqResponses" select="$exclusionCriteria[cac:TenderingCriterionPropertyGroup[cac:TenderingCriterionProperty[cbc:TypeCode='REQUIREMENT']      and cac:SubsidiaryTenderingCriterionPropertyGroup/cac:TenderingCriterionProperty[cbc:ID = $allResponses and cbc:TypeCode='QUESTION'] ]]/cbc:CriterionTypeCode" />
    <xsl:variable name="exclusionNotResponses" select="$exclusionCriteria[cac:TenderingCriterionPropertyGroup/cac:TenderingCriterionProperty[not(cbc:ID = $allResponses) and cbc:TypeCode='QUESTION']]/cbc:CriterionTypeCode/text()" />
    <xsl:variable name="exclusionNotReqResponses" select="$exclusionCriteria[cac:TenderingCriterionPropertyGroup[cac:TenderingCriterionProperty[cbc:TypeCode='REQUIREMENT']      and cac:SubsidiaryTenderingCriterionPropertyGroup/cac:TenderingCriterionProperty[not(cbc:ID = $allResponses) and cbc:TypeCode='QUESTION'] ]]/cbc:CriterionTypeCode" />

		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="($isPQS) or(not($isPQS) and (count($exclusionCriteria) = (count($exclusionResponses) + count($exclusionReqResponses))) )" />
      <xsl:otherwise>
        <svrl:failed-assert test="($isPQS) or(not($isPQS) and (count($exclusionCriteria) = (count($exclusionResponses) + count($exclusionReqResponses))) )">
    ...
    <xsl:variable name="selectionResponses" select="$selectionCriteria[cac:TenderingCriterionPropertyGroup/cac:TenderingCriterionProperty[cbc:ID = $allResponses and cbc:TypeCode='QUESTION']]/cbc:CriterionTypeCode" />
    <xsl:variable name="selectionReqResponses" select="$selectionCriteria[cac:TenderingCriterionPropertyGroup[cac:TenderingCriterionProperty[cbc:TypeCode!='QUESTION']      and cac:SubsidiaryTenderingCriterionPropertyGroup//cac:TenderingCriterionProperty[cbc:ID = $allResponses and cbc:TypeCode='QUESTION'] ]]/cbc:CriterionTypeCode" />
    <xsl:variable name="selectionNotResponses" select="$selectionCriteria[cac:TenderingCriterionPropertyGroup/cac:TenderingCriterionProperty[not(cbc:ID = $allResponses) and cbc:TypeCode='QUESTION']]/cbc:CriterionTypeCode" />
    <xsl:variable name="selectionNotReqResponses" select="$selectionCriteria[cac:TenderingCriterionPropertyGroup[cac:TenderingCriterionProperty[cbc:TypeCode!='QUESTION']      and count(cac:SubsidiaryTenderingCriterionPropertyGroup//cac:TenderingCriterionProperty[cbc:ID = $allResponses and cbc:TypeCode='QUESTION'])=0 ]]/cbc:CriterionTypeCode" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="(($isPQS) and not($isOENRON)) or (not(($isPQS) and not($isOENRON)) and (count($selectionCriteria) = (count($selectionResponses) + count($selectionReqResponses))) )">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($testS10) or ($testS10 and (count($selectionCriteria) = (count($selectionResponses) + count($selectionReqResponses))) )">
